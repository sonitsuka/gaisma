...
let textPosition = 20; // starting Y position for text
...
canvas.addEventListener('click', function(e) {
    ...
    textInput.addEventListener('input', function() {
        ...
        textInput.size = Math.max(1, textInput.value.length); // at least 
    });
...
textInput.addEventListener('keydown', function(e) {
    if (e.key === 'Enter') {
        ...
        if (text !== '') {
            ...
            textPosition += 30; // move to next line for next text
            saveCanvas();
        }
        // remove the text input field
        textInput.parentNode.removeChild(textInput);
...
document.getElementById('eraseButton').addEventListener('click', function() {
    mode = 'erase';
    erasingText = true; // enable erasing text
});
